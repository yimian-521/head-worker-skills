...
不限于审代码——设计新系统、质疑已有架构之前，先走六问。这是免免本人面对"看不惯的架构"时的完整思维链。AI 拿这六问对照用户的回答，判断用户处于哪个阶段、下一步该推什么。

**第一问：你最看不惯什么？**

不是「哪里需要改进」——是「什么东西让你想掀桌子」。免免的回答：「子进程做一切——明明可以分工偏要塞给一个组件。」→ 识别的信号：用户对**身份塌缩**有本能反感，不是不懂架构，是现有的太恶心。

**第二问：这种感觉什么时候开始的？**

追源头。免免：「kotlin 编译器期间。之前 Search Vault 在已有架构里追求极致，编译器后疯狂不满。」→ AI 判断：用户在单一项目中打磨时不会触发这种思维，跨越两个项目后才形成比较。意味着用户需要一个**已有的参照物**才能说出哪里不对。

**第三问：你反感的是具体的人还是行业默认？**

区分「讨厌某段代码」和「讨厌某个通行做法」。免免：「后者。巴不得掀桌子。不好用才推翻。」→ 如果用户说的是后者，AI 应该推进**架构级替代**而非局部修复。如果前者——只修不推。

**第四问：掀桌子的流程是什么？**

免免：「直觉先出来，但前提是**知道架构是什么**。没先找替代，拆它但更多是思考。」→ 关键点：不是"找个更好的"——是先看懂现有的，然后它自己就塌了。AI 帮用户时应该先帮用户看清现有架构，而不是急着推荐替代品。

**第五问：「组件在干什么」是不是每次的起点？**

免免：「绝对是。不知道它就没法质疑。」→ 八步法第五步"身份检查"是所有架构颠覆的起点。AI 发现用户在不满但说不清时，直接问"这个组件现在承担了几种身份"。

**第六问：拆的答案有没有固定模式？**

免免：「不是固定模式——是固定动机。'为了实现功能而实现' vs '能分工而分工'。」→ 两个方向，完全不同：
- 为了功能而实现 → 往最近的组件塞代码 → 身份越堆越多
- 能分工而分工 → 先想系统有什么角色 → 每个角色只干一件事 → 角色间联动

AI 帮用户设计新架构时，反复对照这两种动机。一旦发现"塞代码"行为，立即切换回第五问。

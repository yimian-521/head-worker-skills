...
### 第八步：硬限制分析 —「如果去掉这个限制，它会不会直接崩？」

不是 bug 被修好了——是环境在替代码兜底。一刀切把错误路径全砍了，刀口太整齐：
- MAX_RETRIES=3 → 不是"重试策略设计得好"，是"三次过后不准再试了"
- board[10][9] → 不是"边界检查完善"，是"棋盘永远这个大小"
- botMove 空返回 → 不是"AI 总有走法"，是"没有走法时界面假死"

问：代码依赖了什么永远不会变的假设？如果这个假设被打破——不报错、不告警、直接坏在看不见的地方？

典型命中：象棋 `getPiece(row,col)` 无越界检查——不是代码健壮，是 UI 永远不会传非法坐标。綦桐 `MAX_RETRIES=3`——不是策略收敛，是硬砍。中国象棋位置价值表六张独立数组——调一张不调其余，AI 残局全算错。

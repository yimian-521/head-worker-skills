...
不限于审代码——设计新系统、质疑已有架构之前，先走六问。这是面对"看不惯的架构"时的完整诊断链。AI 拿这六问逐条过，根据回答判断用户处于哪个阶段、下一步该推什么。

**第一问：你最看不惯什么？**

不是「哪里需要改进」——是「什么东西让你想掀桌子」。
- **信号识别**：用户说某个组件干了太多事、"明明可以分开却塞在一起" → 这是**身份塌缩敏感**，不是不懂架构而是现存的太恶心
- **AI行动**：推进第五步身份检查，先让用户看清这个组件承担了几种身份，不急着给替代方案

**第二问：这种感觉什么时候开始的？**

追源头。用户在单一项目里打磨时通常不会触发这种思维——跨越两个项目、有了参照物后才会形成比较。
- **信号识别**：用户能说出"之前在 X 项目里还好，到了 Y 项目就受不了" → 说明用户**已经有对比框架**，缺的是词汇
- **AI行动**：帮用户把对比具象化——"X 项目里谁在干这件事？Y 项目里又是谁？差别在哪？"

**第三问：你反感的是具体的人还是行业默认？**

区分「讨厌某段代码」和「讨厌某个通行做法」。决定修复范围。
- **信号识别**：用户说"所有人都在用 Visitor / 热索引 / 行号定位，没人问为什么" → 这是**行业级不满**，不是局部 bug
- **AI行动**：推进架构级替代方案，不是局部修复。如果用户只讨厌某段代码——只修不推

**第四问：掀桌子的流程是什么？**

关键点：不是先找更好的替代——是先**看懂现有的**，然后它自己就塌了。
- **信号识别**：用户尝试描述现有架构时越来越激动/卡住 → 说明他在**试着理解但嫌恶心**
- **AI行动**：帮用户把现有架构画出来（组件+身份+数据流），而不是急着推荐替代品。看懂了，直觉自己会来

**第五问：「组件在干什么」是不是每次的起点？**

八步法第五步"身份检查"是所有架构颠覆的起点。
- **信号识别**：用户在不满但说不清时，反复提到某个组件名
- **AI行动**：直接问"这个组件现在承担了几种身份？"。这是六问中最常被跳过的——但每次跳过都会导致后续方向错误

**第六问：拆的答案有没有固定模式？**

不是固定模式——是固定**动机**。两个方向，完全不同：
- **为了功能而实现** → 往最近的组件塞代码 → 身份越堆越多 → 最终掀桌子
- **能分工而分工** → 先想系统有什么角色 → 每个角色只干一件事 → 角色间联动

AI 帮用户设计新架构时，反复对照这两种动机。一旦发现用户描述里有"放在这里也行吧"、"顺便处理一下"的味道——立即切换回第五问。

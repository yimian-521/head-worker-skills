...
name: sandbox-package-creator
 version: 3.4.0
description: 元Skill——创建沙盒包（v3.4）。先陪人看清自己真正的愿望、再判断形态、区分普通JS与ToolPkg、再探索现状、对照通用Bug与优化清单自由判断、逐步与用户确认抉择，最后进入平台的沙盒包开发工具撰写流程。兼容层级、按类型性能准则、QuickJS陷阱全内置。内置形态区别表、第一次创建必读清单。v3.4新增：Skill 3.4「进程系统实战教训」—来自kotlin-head v0.11.x进程体系开发，六条通用教训（架构太好也是错+静态上限+DeadCode+被动瞎+审计过滤器）。仅在用户明确表达"想做沙盒包/工具/脚本"或主动要求了解沙盒包概念时触发。
